    ...
    <Wrap>
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-stone-500 text-[12px] mt-1">{pendingCount} por revisar · {centros.length} centros · {contactos.length} mensaje{contactos.length !== 1 ? "s" : ""}</p>
        </div>
        <button onClick={() => supabase.auth.signOut()} className="text-stone-400 text-[12px]">Salir</button>
      </div>

      <div className="mt-4 flex gap-1 rounded-xl border border-stone-200 bg-stone-100 p-1">
        {([["centros", `Centros (${centros.length})`], ["mensajes", `Mensajes (${contactos.length})`]] as const).map(([k, label]) => (
          <button
            key={k}
            onClick={() => setTab(k)}
